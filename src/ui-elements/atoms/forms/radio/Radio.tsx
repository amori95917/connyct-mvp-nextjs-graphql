...
export const Radio: React.FC<RadioProps> = forwardRef<HTMLInputElement, RadioProps>(
	({ inputClassName = '', id, name, ...props }, ref) => {
		...
		const _class =
			'form-check-input appearance-none h-6 w-6 rounded-full checked:bg-primary checked:border-primary focus:outline-none transition duration-200 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer';
		return (
			<input
				name={name}
				ref={ref}
